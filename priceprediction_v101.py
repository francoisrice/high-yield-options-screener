...
class PricePredictor:
    ...
    def __init__(self, symbol="", dailyVolatility=None, volatility=None, stockData=None, currentPrice=None, drift=None) -> None:
        self.symbol = symbol
        self.dailyVolatility = None
    ...
    def calculate_safe_short_call_price(self, symbol, probability, daysToExpiration):
        # Calculate the price that has a probability % chance of being above the price in daysToExpiration days
        self.symbol = symbol
        self.fetch_stock_data()
        self.calculate_volatility()
        ...
        topPrice = np.exp(zScoreA * periodVol + self.avgReturn) * meanEndPrice

        return topPrice        
    
    def calculate_safe_short_put_price(self, symbol, probability, daysToExpiration):
        # Calculate the price that has a probability % chance of being above the price in daysToExpiration days
        self.symbol = symbol
        self.fetch_stock_data()
        self.calculate_volatility()
        ...
        bottomPrice = np.exp(zScoreB * periodVol + self.avgReturn) * meanEndPrice

        return bottomPrice
    
    def calculate_price_probability(self, symbol, price, daysToExpiration, volatility = None):
        self.symbol = symbol
        self.fetch_stock_data()
        self.calculate_volatility()
        self.calculate_drift()
